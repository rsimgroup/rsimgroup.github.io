...
void write_frame_buf8(unsigned char data) {
...
void write_frame_buf32(unsigned int data) {
  write_frame_buf8((unsigned char) (data >> 24));
  write_frame_buf8((unsigned char) (data >> 16));
  write_frame_buf8((unsigned char) (data >> 8));
  write_frame_buf8((unsigned char) data);
}
...
int framenum, MBAmax;
{
  ...
  ptr_offset[0] = frame_buf_offset = 0;
  ...
  thread_data_array[0].num_slices = num_decode_slice[0];
  thread_data_array[0].framenum = framenum;
  thread_data_array[0].MBAmax = MBAmax;
  thrd_ptr[0] = frame_buffer+ptr_offset[0];
  
#if 0
  Thrd_Initialize_Buffer(0);

  if (0!=num_thrds-1) {
    rc = pthread_create(&thread[0], &attr, Thrd_Work, 
  ...
  while (num_slices <= mb_height) {
    ...
    while (Show_Bits(24)!=0x01L) {
      write_frame_buf8((unsigned char)Show_Bits(8));
      Flush_Buffer(8);
    };
	...
	ptr_offset[thrd_num] = frame_buf_offset;
...
#if 0	
	/*Thread Create */
	thread_data_array[thrd_num].id = thrd_num;
	thread_data_array[thrd_num].num_slices = num_decode_slice[thrd_num];
	thread_data_array[thrd_num].framenum = framenum;
	thread_data_array[thrd_num].MBAmax = MBAmax;
	thrd_ptr[thrd_num] = frame_buffer+ptr_offset[thrd_num];
	
	Thrd_Initialize_Buffer(thrd_num-1);
	
	if (thrd_num-1!=num_thrds-1) {
	  rc = pthread_create(&thread[thrd_num-1], &attr, Thrd_Work, 
			      (void*) &thread_data_array[thrd_num-1]); 
	  if (rc) {
	    printf("ERROR; return code from pthread_create() is %d\n", rc);
	    exit(-1);
	  }
...
#endif
  Thrd_Initialize_Buffer(num_thrds-1);

  Thrd_Work((void*) &thread_data_array[num_thrds-1]);
