...
int mquant;
{
...
#ifdef SSE2
  {
...
#if (QUANTIZE_TEST_CODE)
    short _dst[64],_src[64];
    ...
    for (i=0;i<64;i++)
      if (dst[i]!=_dst[i]) {
	printf("qi %d good %d bad %d\n",i,_dst[i],dst[i]);
	printf("      src %d qm %d mq %d\n",_src[i],quant_mat[i],mquant);
	x = _src[i];
	d = quant_mat[i];
	y = (32*(x>=0 ? x : -x) + (d>>1))/d;
	printf("y %d\n",y);
...
#if (QUANTIZE_TEST_CODE)
    short _dst[64],_src[64],_nzflag=0;
    for (i=0; i<64; i++) {
      x = src[i];
      d = quant_mat[i];
...
#if (QUANTIZE_TEST_CODE)
    if (nzflag!=_nzflag) printf("bad nzflag nz %d _nz %d\n",nzflag, _nzflag);
...
int mquant;
{
  ...
  if (mpeg1)
	...
	val = (int)(src[i]*quant_mat[i]*mquant)>>4;/*/16;*/
	sum_chk^= chk[i] = (val>2047) ? 2047 : ((val<-2048) ? -2048 : val);
	i++;
