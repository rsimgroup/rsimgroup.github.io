...
#ifdef NEW_PAR
    for (s = t; s < n_mgau; s+=NUM_THREADS) {
...
#ifdef NEW_PAR
  int32 n_comstate = my_data->d2p->n_comstate;

  for (i = thrd_args->id; i < n_comstate; i+=NUM_THREADS) {
#else
  for (i = start; i < end; i++) {
#endif
...
#if 0 /*def NEW_PAR*/
    for (i = t;i<n_active;i+=NUM_THREADS) {
#else
    for (i = start; i < end; i++) {
#endif
      ln = list[i];
...
int32 ln_lock(int32 t1, lextree_node_t*ln)
{
  ...
  do 
	...
	ln->synch = t1;
      /*if (skip) printf("thread %d skips\n",t1-1);
	if (retry) printf("thread %d retries\n",t1-1);*/
