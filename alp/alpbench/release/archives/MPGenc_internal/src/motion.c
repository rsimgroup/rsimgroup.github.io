     ...
     int distlim;
{
  ...
  if (!hx && !hy){
	...
	dist1_x0_y0:
	  ...
	  paddusw xmm2, xmm0    ; /* keep 2 partial sum */
	  add     ebx, eax      ;
	  add     edx, eax      ;
	  sub     ecx, 1        ;
	  cmp     ecx, 0        ;
  ...
  for (j=0; j<h; j++) {
   ...
   if (s!=sa) {
     ...
     printf("d1_11:bad %d good %d h %d\n",s,sa,h);
     p1 = blk1; p2 = blk2; p1a=p1+lx;
     
     for (j=0; j<h; j++) {
       for (i=0; i<16; i++) {
	 v = ((unsigned int)(p1[i]+p1[i+1]+p1a[i]+p1a[i+1]+2)>>2) - p2[i];
	 v = (v<0)?-v:v;
	 sa = ((unsigned int)((((p1[i]+p1[i+1]+1)>>1)+((p1a[i]+p1a[i+1]+1)>>1)+1)>>1))-p2[i];
	 sa = (sa<0)?-sa:sa;
	 if (v!=sa) {
	   acc += v-sa;
	   printf("v %d sa %d\n",v,sa);
	 }
	 //printf("p1 %d p1+ %d p1a %d p1a+ %d p2 %d v %d\n",p1[i],p1[i+1],p1a[i],p1a[i+1],p2[i],v);
       }
      p1 = p1a;
      p1a+= lx;
      p2+= lx;
